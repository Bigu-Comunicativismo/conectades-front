...
export const Route = createFileRoute('/donations/_Donation/$id')({
    ...
    loader: async ({ params }) => {
            ...
            const DonationFetched: DonationFetched = await apiFetch<DonationFetched>({ apiPath: `http://srv1037558.hstgr.cloud:8001/api/doacoes/independentes/${campaignId}` })
            .catch(() => {
  ...
  return (
      <Section classCss={styles.campaignSection} ref={activeSection}>
        ...
        <Container classCss={styles.campaignContainer}>
            {activeItem === 'Sobre' ? <About campaign={DonationFetched.descricao}/> : activeItem === 'Organização' ? <Organization organizator={{name: DonationFetched.doarora_nome, avatar: DonationFetched.doadora.avatar, location: `${DonationFetched.doadora.bairro} ${DonationFetched.doadora.cidade}`, type: DonationFetched.doadora.tipo_usuario, contact: DonationFetched.doadora.telefone}}/> : null}
        </Container>
