...
import ItemsList from "../New/Item";
...
import { formEditCampaign } from "@/utils/formEditCampaing";
...
//     }
    ...
    const [title, setTitle] = useState(campaign.titulo);
    const [subtitle, setSubtitle] = useState(campaign.subtitulo);
    const [description, setDescription] = useState<string>(campaign.descricao);
    const [profileImage, setProfileImage] = useState<File | null>(null);
    const [previewUrl, setPreviewUrl] = useState<string>(`${imgBaseUrl}${campaign.imagem_url}`);
    const [categories, setCategories] = useState<category[] | never[]>([]);
    const [markedCategories, setMarkedCategories] = useState<number[] | never>(campaign.categorias);
    const [items, setItems] = useState(campaign.itens);
    // const [isOnlyOrganizer, setIsOnlyOrganizer] = useState(false);
    // const [city, setCity] = useState<string>("");
    // const [neighborhood, setNeighborhood] = useState<string>("");
    // const [neighborhoods, setNeighborhoods] = useState<Neighborhood[] | []>([]);
    // const [cities, setCities] = useState<any[] | []>([]);
    const [contact, setContact] = useState<string>(campaign.whatsapp);
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      const handleSubmit = async () => {
        ...
        const editedCampaign = new Campaign(title, subtitle, description, retriedUser.id, profileImage, markedCategories, contact, Number(campaign.localizacao), campaign.data_inicio, campaign.prazo, true, JSON.stringify(items));
  
        const response = formEditCampaign(editedCampaign, campaign.id);
    ...
    return (
        <Container classCss={styles.container} ref={activeSection}>
            {isOnFocus ? <ButtonGroupFloating btn1Text="Cancelar" btn2Text="Salvar" disebled={((!!title) && (!!subtitle) && (!!description) && (!!markedCategories) && (!!items)) && (!!contact) ? false : true} btn1Action={() => navigate({ to: "/" })} btn2Action={handleSubmit} /> : <div></div>}
            <Title.Level1 text="Editar campanha" classCss={styles.title} />
