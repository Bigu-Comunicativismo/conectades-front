...
import { formEditDonation } from "@/utils/formEditDonation";
...
export function EditDonation({campaign}: any) {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      const handleSubmit = async () => {
        ...
        const editedCampaign = new Campaign(title, subtitle, description, retriedUser.id, profileImage, markedCategories, contact, Number(campaign.localizacao), campaign.data_inicio, campaign.data_fim, true, JSON.stringify([]));
  
        const response = formEditDonation(editedCampaign, campaign.id);
    ...
    return (
        <Container classCss={styles.container} ref={activeSection}>
            {isOnFocus ? <ButtonGroupFloating btn1Text="Cancelar" btn2Text="Salvar" disebled={((!!title) && (!!subtitle) && (!!description) && (!!markedCategories)) && (!!contact) ? false : true} btn1Action={() => navigate({ to: "/" })} btn2Action={handleSubmit} /> : <div></div>}
            <Title.Level1 text="Editar doação" classCss={styles.title} />
