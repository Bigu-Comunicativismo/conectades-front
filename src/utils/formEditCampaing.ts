...
export async function formEditCampaign<T extends Record<string, any>>(obj: T, id: number) {
  ...
  const createFormData = (obj: any) => {
    ...
    try{
        ...
        const response = await apiFetch({apiPath: `http://srv1037558.hstgr.cloud:8001/api/campanhas/${id}/editar/`, apiMethod: 'PUT', apiBody: data, apiHeaders: { "Authorization": `Bearer ${token.access}` }});
